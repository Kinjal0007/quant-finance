...
def load_fixture_prices(
    ...
    fixture_path = os.path.join(os.path.dirname(__file__), "fixtures", "prices_demo.csv")
    ...
    wide_df = df.pivot(index='date', columns='symbol', values='adjusted_close' if adjusted else 'close')
...
def _generate_monte_carlo_metrics(
    ...
    return {
        ...
        "simulation_paths": cumulative_returns.tolist()[:10]  # First 10 paths for visualization
    }
...
def _generate_black_scholes_metrics(
    ...
    if option_type == "call":
        option_price = max(current_price - strike_price, 0) + volatility * time_to_expiry * 0.4
        delta = 0.6 if moneyness > 1 else 0.4
    else:  # put
        option_price = max(strike_price - current_price, 0) + volatility * time_to_expiry * 0.4
